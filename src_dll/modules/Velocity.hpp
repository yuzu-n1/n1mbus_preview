...
public:
    float horizontal = 0.0f; // 0.0f to 100.0f
    float vertical = 0.0f;   // 0.0f to 100.0f
    float chance = 100.0f;   // 0.0f to 100.0f
        ...
        if (m_hurtTimeF && m_motionXF && m_motionYF && m_motionZF) {
            ...
            if (hurtTime == 10 && m_lastHurtTime != 10) {
                ...
                if (dis(gen) <= chance) {
                    ...
                    env->SetDoubleField(playerObj, m_motionXF, mx * (horizontal / 100.0f));
                    env->SetDoubleField(playerObj, m_motionYF, my * (vertical / 100.0f));
                    env->SetDoubleField(playerObj, m_motionZF, mz * (horizontal / 100.0f));
                }
            }
