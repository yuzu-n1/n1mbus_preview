...
public:
    ...
    // Reflection-based search: find a method that takes exactly one Entity parameter
    // and returns boolean. Works regardless of class name obfuscation.
    static jmethodID FindIsOnSameTeamByReflection(JNIEnv* env, jclass entityClass) {
        jclass classClass = env->FindClass("java/lang/Class");
        ...
        if (!getDeclaredMethods) { env->DeleteLocalRef(classClass); return nullptr; }

        jobjectArray methods = (jobjectArray)env->CallObjectMethod(entityClass, getDeclaredMethods);
        if (env->ExceptionCheck()) { env->ExceptionClear(); env->DeleteLocalRef(classClass); return nullptr; }
        if (!methods) { env->DeleteLocalRef(classClass); return nullptr; }
        ...
        jmethodID getNameM       = env->GetMethodID(methodClass, "getName",           "()Ljava/lang/String;");
        jclass    boolPrimClass  = nullptr;
        {   // Get boolean primitive class via Boolean.TYPE
            jclass boolClass = env->FindClass("java/lang/Boolean");
            jfieldID typeField = env->GetStaticFieldID(boolClass, "TYPE", "Ljava/lang/Class;");
            boolPrimClass = (jclass)env->GetStaticObjectField(boolClass, typeField);
            env->DeleteLocalRef(boolClass);
        }

        jmethodID result = nullptr;
        jsize len = env->GetArrayLength(methods);
        for (jsize i = 0; i < len && !result; i++) {
            jobject m = env->GetObjectArrayElement(methods, i);
            if (!m) continue;

            // Must return boolean
            jclass retType = (jclass)env->CallObjectMethod(m, getReturnType);
            bool retBool = retType && env->IsSameObject(retType, boolPrimClass);
            if (retType) env->DeleteLocalRef(retType);
            if (!retBool) { env->DeleteLocalRef(m); continue; }

            // Must take exactly 1 parameter
            jobjectArray params = (jobjectArray)env->CallObjectMethod(m, getParamTypes);
            jsize paramCount = params ? env->GetArrayLength(params) : 0;
            if (paramCount != 1) {
            ...
            // Parameter must be an Entity (i.e., same class or superclass of entityClass)
            jobject param0 = env->GetObjectArrayElement(params, 0);
            bool paramIsEntity = param0 && env->IsSameObject(param0, entityClass);
            // Also accept if entityClass isAssignableFrom param0
            if (!paramIsEntity && param0) {
                jmethodID isAssignable = env->GetMethodID(classClass, "isAssignableFrom", "(Ljava/lang/Class;)Z");
                jboolean ok = env->CallBooleanMethod((jobject)param0, isAssignable, entityClass);
                if (env->ExceptionCheck()) env->ExceptionClear();
                paramIsEntity = (ok == JNI_TRUE);
            }
            if (param0) env->DeleteLocalRef(param0);
            env->DeleteLocalRef(params);

            if (!paramIsEntity) { env->DeleteLocalRef(m); continue; }

            // Get the method name and resolve via GetMethodID
            jstring nameStr = (jstring)env->CallObjectMethod(m, getNameM);
            const char* nameChars = env->GetStringUTFChars(nameStr, nullptr);
            std::string mName = nameChars;
            env->ReleaseStringUTFChars(nameStr, nameChars);
            env->DeleteLocalRef(nameStr);
            env->DeleteLocalRef(m);

            // Build the actual JNI signature using the entity class itself
            // Get the class name to build the sig
            jmethodID getNameCls = env->GetMethodID(classClass, "getName", "()Ljava/lang/String;");
            jstring clsNameStr = (jstring)env->CallObjectMethod(entityClass, getNameCls);
            const char* clsNameChars = env->GetStringUTFChars(clsNameStr, nullptr);
            std::string clsName = clsNameChars;
            env->ReleaseStringUTFChars(clsNameStr, clsNameChars);
            env->DeleteLocalRef(clsNameStr);

            std::replace(clsName.begin(), clsName.end(), '.', '/');
            std::string sig = "(L" + clsName + ";)Z";

            result = env->GetMethodID(entityClass, mName.c_str(), sig.c_str());
            if (env->ExceptionCheck()) { env->ExceptionClear(); result = nullptr; }

            if (result) {
                // Cache in Mappings
                Mappings::Entity_isOnSameTeam_Name = _strdup(mName.c_str());
                Mappings::Entity_isOnSameTeam_Sig  = _strdup(sig.c_str());
            }
        }

        if (boolPrimClass) env->DeleteLocalRef(boolPrimClass);
        env->DeleteLocalRef(methodClass);
        env->DeleteLocalRef(methods);
        env->DeleteLocalRef(classClass);
        return result;
    ...
    // Call isOnSameTeam safely. Returns false if method not found.
    static bool CallIsOnSameTeam(JNIEnv* env, jobject caller, jobject target, jclass entityClass) {
        static jmethodID cached = nullptr;
        static bool tried = false;
        if (!cached && !tried) {
            tried = true;

            // Get the actual obfuscated class name at runtime to build correct JNI signature
            jclass classClass = env->FindClass("java/lang/Class");
            jmethodID getNameMeth = env->GetMethodID(classClass, "getName", "()Ljava/lang/String;");
            jstring clsNameStr = (jstring)env->CallObjectMethod((jobject)entityClass, getNameMeth);
            const char* clsNameChars = env->GetStringUTFChars(clsNameStr, nullptr);
            std::string clsName = clsNameChars;
            env->ReleaseStringUTFChars(clsNameStr, clsNameChars);
            env->DeleteLocalRef(clsNameStr);
            ...
            // Java class name uses dots; JNI signature needs slashes
            std::replace(clsName.begin(), clsName.end(), '.', '/');
            std::string sig = "(L" + clsName + ";)Z";

            // Try every plausible method name (SRG, deobfuscated, and single-letter obfuscated)
            static const std::vector<std::string> names = {
                "func_142014_c", "isOnSameTeam",
                "a", "b", "c", "d", "e", "f", "g", "h"
            };
            for (const auto& name : names) {
                cached = env->GetMethodID(entityClass, name.c_str(), sig.c_str());
                if (env->ExceptionCheck()) { env->ExceptionClear(); cached = nullptr; continue; }
                if (cached) break;
            }

            // Last resort: reflection scan
            if (!cached) {
                cached = FindIsOnSameTeamByReflection(env, entityClass);
            }
        }
        if (!cached) return false;

        jboolean result = env->CallBooleanMethod(caller, cached, target);
        if (env->ExceptionCheck()) { env->ExceptionClear(); return false; }
        if (result == JNI_TRUE) return true;

        // Fallback: check formatted name color (e.g. Hypixel BedWars)
        std::string pName = GetEntityFormattedName(env, caller);
        std::string eName = GetEntityFormattedName(env, target);
        if (!pName.empty() && !eName.empty()) {
            char pCol = GetTeamColorCode(pName);
            char eCol = GetTeamColorCode(eName);
            if (pCol != '?' && eCol != '?' && pCol == eCol) return true;
        }
    ...
    static int GetColorCodeIndex(char code) {
    ...
    static char GetTeamColorCode(const std::string& formattedText) {
        for (int i = (int)formattedText.length() - 2; i >= 0; i--) {
            if ((unsigned char)formattedText[i] == 0xA7 || formattedText[i] == '&') {
                int idx = GetColorCodeIndex(formattedText[i+1]);
                if (idx >= 0 && idx < 16) return formattedText[i+1];
            }
        }
        return '?';
    }

    static std::string GetEntityFormattedName(JNIEnv* env, jobject entity) {
        jclass entityCls = env->GetObjectClass(entity);
        std::string dnName = FindMethodFromNames(env, entityCls, {"getDisplayName", "func_145748_c_", "f_"}, "()Lnet/minecraft/util/IChatComponent;");
        if (dnName.empty()) { env->DeleteLocalRef(entityCls); return ""; }
        
        jmethodID getDisplayName = env->GetMethodID(entityCls, dnName.c_str(), "()Lnet/minecraft/util/IChatComponent;");
        jobject chatComp = getDisplayName ? env->CallObjectMethod(entity, getDisplayName) : nullptr;
        if (env->ExceptionCheck()) { env->ExceptionClear(); env->DeleteLocalRef(entityCls); return ""; }
        if (!chatComp) { env->DeleteLocalRef(entityCls); return ""; }
        
        jclass chatCls = env->GetObjectClass(chatComp);
        std::string fTextName = FindMethodFromNames(env, chatCls, {"getFormattedText", "func_150254_d", "c"}, "()Ljava/lang/String;");
        if (fTextName.empty()) { env->DeleteLocalRef(chatCls); env->DeleteLocalRef(chatComp); env->DeleteLocalRef(entityCls); return ""; }
        
        jmethodID getFormattedText = env->GetMethodID(chatCls, fTextName.c_str(), "()Ljava/lang/String;");
        jstring formattedJStr = getFormattedText ? (jstring)env->CallObjectMethod(chatComp, getFormattedText) : nullptr;
        if (env->ExceptionCheck()) { env->ExceptionClear(); }
        
        std::string result = "";
        if (formattedJStr) {
            const char* chars = env->GetStringUTFChars(formattedJStr, nullptr);
            if (chars) {
                result = chars;
                env->ReleaseStringUTFChars(formattedJStr, chars);
            }
            env->DeleteLocalRef(formattedJStr);
        }
        
        env->DeleteLocalRef(chatCls);
    ...
    static bool ResolveAll() {
        ...
        if (entityClass) {
            ...
            std::string entitySig = "(L" + DotToSlash(resolvedEntity) + ";)Z";
            static std::string isOnSameTeam_Name = FindMethodFromNames(env, entityClass,
                {"func_142014_c", "isOnSameTeam", "c"}, entitySig.c_str());
            if (!isOnSameTeam_Name.empty()) {
                Mappings::Entity_isOnSameTeam_Name = _strdup(isOnSameTeam_Name.c_str());
                Mappings::Entity_isOnSameTeam_Sig = _strdup(entitySig.c_str());
            }
            Log("isOnSameTeam mapping: %s\n", isOnSameTeam_Name.c_str());
